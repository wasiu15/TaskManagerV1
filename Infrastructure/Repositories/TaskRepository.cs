...
namespace TaskManager.Infrastructure.Repositories
{
    public class TaskRepository : RepositoryBase<UserTask>, ITaskRepository
    {
        ...
        public async Task<UserTask> GetTaskByTaskId(string taskId, bool trackChanges) => await FindByCondition(x => x.TaskId.Equals(taskId), trackChanges).FirstOrDefaultAsync();
        ...
        public async Task<IEnumerable<UserTask>> GetTasksByProjectId(string projectId, bool trackChanges) => await FindByCondition(x => x.ProjectId.Equals(projectId), trackChanges).ToListAsync();
